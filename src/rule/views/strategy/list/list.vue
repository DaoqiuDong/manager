              ...
              <template scope="scope">
                <div v-if="scope.row.count != -1">
                  <div v-if="scope.row.strategyStatus == 0">
                    ...
                    <el-button type="text" @click="release(scope.row.id)" v-if="hasBtnAuth('B60002',btnApiList)" v-text="getbtnName('B60002',btnApiList)"></el-button> 
                  </div>
                  ...
                  <el-button type="text" @click="delStr(scope.row.id)">删除</el-button>
                  <el-button type="text" @click="release(scope.row.id)">发布</el-button>  -->
                </div>
              </template>
              </el-table-column>            
          </el-table>
        ...
        <el-dialog title="添加策略" :visible.sync="addStrDialog" size="tiny">
            <el-form label-width="90px" ref="addForm">
              <el-form-item label="策略名称" prop="strName">
                <el-input v-model="addStrForm.strName" placeholder="策略名称"></el-input>
              </el-form-item>
              ...
              <el-form-item label="策略版本号" prop="version">
                <el-input v-model="addStrForm.version" placeholder="策略版本号"  :maxlength="5"></el-input>
              </el-form-item>
              <el-form-item label="策略框架" prop="selectStr">
                <el-select clearable v-model="addStrForm.selectStr" filterable placeholder="策略框架">
                  <el-option
                    v-for="item in allStrList"
                    :key="item.code"
                    :label="item.name"
                    :value="item.id">
                  </el-option>
...
export default {
  ...
  methods: {
    ...
    addStrDone() {
      ...
      if (this.isEmpty(addStrForm.version)) {
        this.$message({
          message: "策略版本号必填",
          type: "error"
        });
