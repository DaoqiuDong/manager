...
<div class="main_module">
  ...
  <div class="operate">
    <el-button type="primary" @click="add">添加子节点</el-button>
    <el-button type="primary" @click="getRuleList">编辑</el-button>
    <el-button type="primary" @click="modify">修改触发条件</el-button>
  ...
  <div v-show='ruleSetVisibility' class="rule_detail" size="small">
    ...
    <el-form label-width="120px" label-position="top">
      <el-form-item label="选择规则">
        <el-select v-model="ruleSet" filterable placeholder="请选择规则" @change="getruleinfo">
          <el-option v-for="item in ruleList" :label="item.value+item.name" :value="item" :key="item.value"></el-option>
        </el-select>
      </el-form-item>
      ...
      <el-form-item v-for="(param,index) in nowRuleInfo.factorParam" :label="param.name" :key="param.key" v-if="nowRuleInfo&&nowRuleInfo.factorParam&&nowRuleInfo.factorParam.length">
        ...
        <el-select :placeholder="param.name" v-else-if="param.type==1" v-model="ParamVal[index]" @change="set(index)">
        ...
        <el-select :placeholder="param.name" multiple v-else-if="param.type==2" v-model="ParamVal[index]" @change="set(index)">
          <el-option v-for="selectItem in param.selectList" :label="selectItem.name" :value="selectItem.value" :key="selectItem.value"></el-option>
        </el-select>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      this.ajax({
        url: "rule/web/strategy/savestrategy",
        data: {
          treeData
        }
      }).then(res => {
    ...
    getruleinfo(value) {
      ...
      this.ajax({
        ...
        _this.nowRuleInfo = res.data;
        _this.nowFactorParam = [];
        if (res.data.factorParam&&res.data.factorParam.length) {
          for (let j = 0; j < res.data.factorParam.length; j++) {
            _this.nowFactorParam[j] = {
              name: res.data.factorParam[j].name,
              key: res.data.factorParam[j].key,
              value: ''
            };
          };
        } else {
        ...
        _this.nowRuleName = res.data.name;
        _this.queryOutParam = res.data.outParam;
        _this.nowRuleCode = res.data.ruleCode;
      }).catch(err => console.log(err))
    },
