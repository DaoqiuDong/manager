  ...
  <div>
    <el-form label-width="120px">
      ...
      <el-form-item label="渠道设置">
        <el-select v-model="channelList" placeholder="选择渠道" multiple>
  ...
  methods: {
    ...
    update() {
      ...
      this.ajax({
        ...
        data: { accid, ...accInfo, channelList, corpProList }
      }).then(res => {
        this.$message({
          message: "修改账户信息成功",
          type: "success"
    ...
    getAccInfo() {
      ...
      this.ajax({
        ...
        this.channelList = this.isEmpty(res.data.channelList)
          ? []
        ...
        if (res.data.corps == 1) {
          ...
          if (res.data.corpProList && res.data.corpProList.length) {
            ...
            for (let i = 0; i < this.corpProList.length; i++) {
              const element = this.corpProList[i];
              this.getProList(element.id,i)
            }
          }
        }
      });
    ...
    addCorp() {
      if (
        this.corpProList.every(element => {
          return element.id;
        })
      ) {
