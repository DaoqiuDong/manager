  ...
  <div>
    ...
    <div>
      <el-table :data="list" :stripe='true' v-loading.body="loading">
        ...
        <el-table-column label="操作" align="center" v-if="hasBtnAuth('B10020',btnGoList)">
          <template scope="scope">
            <router-link :to="{path:'detail',query:{id:scope.row.id}}">
              <el-button type="text" v-text="getbtnName('B10020',btnGoList)"></el-button>
            </router-link>
          </template>
          </el-table-column>            
      </el-table>
...
export default {
  ...
  methods: {
    ...
    getList(pageNo) {
      ...
      this.loading = true;
      this.ajax({
        url: "credit/web/sys/contract/queryAbolished",
        data: {
          pageSize,
          pageNo,
          ...this.searchForm
