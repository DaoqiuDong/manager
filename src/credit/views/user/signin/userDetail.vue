<template>
  <div class="userDetail_module">
    <el-row :gutter="20" type="flex" align="middle">
      ...
      <el-col :span="4">
        <div>
          <el-button type="danger" @click="changeStatusDialog = true"  v-if="userInfo.status == 1&&hasBtnAuth('B20013',btnApiList)" v-text="getbtnName('B20013',btnApiList)"></el-button>
          <el-button type="success" @click="changeStatusDialog = true" v-if="userInfo.status == 2&&hasBtnAuth('B20014',btnApiList)" v-text="getbtnName('B20014',btnApiList)"></el-button>
        </div>
      </el-col>
    </el-row>
    ...
    <div class="type_wrapper">
      ...
      <el-button type="primary" @click="changeTypeDialog = true" v-if="hasBtnAuth('B20015',btnApiList)" v-text="getbtnName('B20015',btnApiList)"></el-button>
    </div>

    <div>
      <h4>授信记录</h4>
      <el-table :data="quotaBrandInfo.list">
        <el-table-column label="机构" prop="corpName"></el-table-column>
        <el-table-column label="申请产品" prop="productName"></el-table-column>
        <el-table-column label="授信额度" prop="amount" :formatter="(row) => count(row.amount,'元')"></el-table-column>
        <el-table-column label="当前可用额度" prop="availableAmount" :formatter="(row) => count(row.availableAmount,'元')"></el-table-column>
        <el-table-column label="操作">
          <template scope="scope">
            <el-button type="text" v-if="scope.row.status == 1&&hasBtnAuth('B20091',btnApiList)"  v-text="getbtnName('B20091',btnApiList)" @click="handleFreeze(scope.row)"></el-button>
            <el-button type="text" v-if="scope.row.status == 2&&hasBtnAuth('B20092',btnApiList)"  v-text="getbtnName('B20092',btnApiList)" @click="handleUnFreeze(scope.row)"></el-button>
            <span v-if="scope.row.status == 3">已过期</span>
          </template>
        </el-table-column>            
      </el-table>
      <el-pagination layout="prev, pager, next" :total="quotaBrandInfo.total" :page-size="5" v-if="getQuotaBrandList.total > 5" @current-change="(i) => getQuotaBrandList(i)"></el-pagination>

      <el-table :data="quotaApplyInfo.list">
        ...
        <el-table-column label="操作" v-if="hasBtnAuth('B10003',btnGoList)" align="center">
          <template scope="scope">
            <router-link :to="{path:'applyDetail',query:{id:scope.row.flowId,code:scope.row.flowCode,type:4}}">
              <el-button type="text" v-text="getbtnName('B10003',btnGoList)"></el-button>
            </router-link>
          </template>
        </el-table-column> 
      </el-table>
      <el-pagination layout="prev, pager, next" :total="quotaApplyInfo.total" :page-size="5" v-if="quotaApplyInfo.total > 5" @current-change="(i) => getQuotaApplyInfo(i)"></el-pagination>
  </div>
    ...
    <div class="apply_wrapper">
      ...
      <el-table :data="applyList" :stripe='true'>
        ...
        <el-table-column label="初审评分">
          ...
          <el-table-column label="操作" v-if="hasBtnAuth('B10005',btnGoList)" align="center">
            <template scope="scope">
              <router-link :to="{path:'applyDetail',query:{id:scope.row.flowId,code:scope.row.flowCode}}">
                <el-button type="text" v-text="getbtnName('B10005',btnGoList)"></el-button>
              </router-link>
            </template>
          </el-table-column> 
      </el-table>
    ...
    <div class="contract_wrapper">
      ...
      <el-table :data="contractList" :stripe='true'>
        ...
        <el-table-column label="操作" align="center" v-if="hasBtnAuth('B10044',btnGoList)" >
          <template scope="scope">
            <router-link  :to="{path:'contractDetail',query:{id:scope.row.id}}">
              <el-button type="text" v-text="getbtnName('B10044',btnGoList)"></el-button>
            </router-link>
          </template>
        </el-table-column> 
      </el-table>
...
export default {
  data() {
    return {
      ...
      remarkDialog: false,
      quotaBrandInfo: {}, //授信机构列表
      quotaApplyInfo: {} //授信申请列表
    };
  },
  mounted() {
    this.getUserInfo();
    this.getRefuseList();
    this.getQuotaBrandList(1);
    this.getQuotaApplyInfo(1);
  },
  computed: {
  ...
  methods: {
    ...
    getUserInfo() {
      ...
      const userAppId = this.$route.query.appId;
      this.ajax({
    ...
    getQuotaBrandList(pageNo) {
      const h5Uid = this.$route.query.userId;
      const pageSize = 5;
      this.ajax({
        url: "credit/web/sys/user/quota/list",
        data: { h5Uid, pageNo, pageSize }
      }).then(res => {
        this.quotaBrandInfo = res.data;
      });
    },
    getQuotaApplyInfo(pageNo) {
      const h5Uid = this.$route.query.userId;
      const flowTypeList = [4];
      const pageSize = 5;
      this.ajax({
        ...
        this.quotaApplyInfo = res.data;
      });
    },
    handleFreeze(row) {
      const h5Uid = this.$route.query.userId;
      const quotaId = row.id;
      this.ajax({
        url: "credit/web/sys/user/quota/update/freeze",
        data: { h5Uid, quotaId }
      }).then(res => {
        this.getQuotaBrandList(1);
        this.$message("冻结成功");
    ...
    handleUnFreeze(row) {
      ...
      const quotaId = row.id;
      this.ajax({
        url: "credit/web/sys/user/quota/update/unfreeze",
        data: { h5Uid, quotaId }
      }).then(res => {
        this.getQuotaBrandList(1);
        this.$message("解冻成功");
