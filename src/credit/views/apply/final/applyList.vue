    ...
    <div>
        ...
        <div>
          <el-table :data="applyList" :stripe='true' v-loading.body="loading">
            ...
            <el-table-column label="操作" v-if="hasBtnAuth('B10011',btnGoList)" align="center">
              <template scope="scope">
                <router-link :to="{path:'applyDetail',query:{id:scope.row.flowId,code:scope.row.flowCode,type:scope.row.flowType}}">
                  <el-button type="text" v-text="getbtnName('B10011',btnGoList)"></el-button>
                </router-link>
              </template>
            </el-table-column>     
          </el-table>
...
export default {
  ...
  methods: {
    ...
    getList(pageNo) {
      ...
      this.loading = true;
      this.ajax({
        url: "credit/web/sys/flow/queryFinalTrial",
        data: {
          pageSize,
          pageNo,
          ...this.searchForm
