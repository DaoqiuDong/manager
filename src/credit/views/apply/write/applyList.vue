    ...
    <div>
        ...
        <div>
          <el-table :data="applyList" v-loading.body="loading" :stripe='true'>
            ...
            <el-table-column label="完成节点" min-width="100" show-overflow-tooltip >
              <template scope="scope">
                <span v-if="scope.row.finishNode&&scope.row.finishNode.length" v-text="scope.row.finishNode.join(',')"></span>
                <span v-else>--</span>
              </template>
            </el-table-column>
            <el-table-column label="进入节点时间" prop="flowCreateTime" min-width="120"></el-table-column>
            ...
            <el-table-column label="操作" v-if="hasBtnAuth('B10007',btnGoList)" align="center">
              <template scope="scope">
                <router-link :to="{path:'applyDetail',query:{id:scope.row.flowId,code:scope.row.flowCode,type:scope.row.flowType}}">
                  <el-button type="text" v-text="getbtnName('B10007',btnGoList)"></el-button>
                </router-link>
              </template>
            </el-table-column>        
          </el-table>
...
export default {
  ...
  methods: {
    ...
    getList(pageNo) {
      ...
      this.loading = true;
      this.ajax({
        url: "credit/web/sys/flow/queryUserInfo",
        data: {
          pageSize,
          pageNo,
          ...this.searchForm
