    ...
    <div>
        ...
        <div>
          <el-table :data="applyList" v-loading.body="loading" :stripe='true'>
            ...
            <el-table-column label="操作" v-if="hasBtnAuth('B10009',btnGoList)" align="center">
              <template scope="scope">
                <router-link :to="{path:'applyDetail',query:{id:scope.row.flowId,code:scope.row.flowCode,type:scope.row.flowType}}">
                  <el-button type="text" v-text="getbtnName('B10009',btnGoList)"></el-button>
                </router-link>
              </template>
            </el-table-column>         
          </el-table>
...
export default {
  ...
  methods: {
    ...
    getList(pageNo) {
      ...
      this.loading = true;
      this.ajax({
        url: "credit/web/sys/flow/queryFirstTrial",
        data: {
          pageSize,
          pageNo,
          ...this.searchForm
