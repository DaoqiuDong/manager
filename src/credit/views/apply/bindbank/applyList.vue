    ...
    <el-form :inline='true'>
      ...
      <el-form-item>
        <el-select clearable v-model="searchForm.nodeStatus" placeholder="节点状态" @change="getList(1)">
          <el-option label="待绑定" value="0"></el-option>              
    ...
    <div>
      <el-table :data="applyList" v-loading.body="loading" :stripe='true'>
        ...
        <el-table-column label="当前节点状态">
          <template scope="scope">
            <span v-if="scope.row.nodeStatus == 0">待绑定</span>
        ...
        <el-table-column label="操作" v-if="hasBtnAuth('B10010',btnGoList)" align="center">
          <template scope="scope">
...
export default {
      ...
      startTimeOption: {
        disabledDate(time) {
          return (
            ...
            time.getTime() >
              new Date(_this.searchForm.nodeEndTime).getTime() - 8.64e7
          );
        }
      },
      endTimeOption: {
  ...
  methods: {
    ...
    getList(pageNo) {
      ...
      this.loading = true;
      this.ajax({
        url: "credit/web/sys/flow/queryBankCard",
        data: {
          pageSize,
          pageNo,
          ...this.searchForm
